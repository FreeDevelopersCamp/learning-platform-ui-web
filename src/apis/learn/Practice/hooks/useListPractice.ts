...
export function useListPractice(instructorId) {
  ...
  const {
    ...
    queryKey: ['practices', instructorId],
    queryFn: () => Practice.getInstance().listByInstructor(instructorId),
  ...
  if (currentPage < pageCount) {
    queryClient.prefetchQuery({
      queryKey: ['practices', instructorId, currentPage + 1],
      queryFn: () =>
        Practice.getInstance().listByInstructor(instructorId, {
          params: {
            page: currentPage + 1,
            limit: PAGE_SIZE,
          },
